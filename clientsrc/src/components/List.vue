    ...
    <div class="row text-center">
      <div class="col-12 listTasks">
        <task draggable="true" v-for="(task, index) in tasks" :key="task.id" :taskData="task" :listId="listData.id"
          @dragstart="reorderTask(task, index)"></task>
      </div>
    </div>
    ...
    <div class="row text-center">
      <div class="col-12">
        <input class="rounded shadow w-100 bg-warning my-1" type="text" placeholder="New task title..."
          v-model="newTask.title" onfocus="this.value=''" />
        <div class="input-group mb-3">
          ...
          <div class="input-group-append">
            <button class="btn btn-outline-light btn-success" type="button" id="button-addon2" @click="addTask()">
              Add
              Task
            </button>
          </div>
